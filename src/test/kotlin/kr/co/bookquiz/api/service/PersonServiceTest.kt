...
class PersonServiceTest {
    ...
    fun `should cleanup unused persons`() {
        ...
        given(personRepository.countBooksByAuthorId(3L)).willReturn(1L) // person3 is still used as author
    ...
    fun `should not delete person that is used in other books`() {
        ...
        // Then
        then(personRepository).should(never()).deleteById(1L) // person1 should not be deleted (used as author)
        then(personRepository).should(never()).deleteById(2L) // person2 should not be deleted (used as translator)
        then(personRepository).should(never()).deleteById(3L) // person3 should not be deleted (used as illustrator)
    }
}
