    ...
    fun handleApiException(e: ApiException): ProblemDetail {
        val status =
                when (e.errorCode) {
                    ...
                    ErrorCode.INVALID_CREDENTIALS, ErrorCode.AUTHENTICATION_FAILED ->
                            HttpStatus.UNAUTHORIZED
                    ErrorCode.ACCESS_DENIED -> HttpStatus.FORBIDDEN
                    ErrorCode.INVALID_TOKEN, ErrorCode.REFRESH_TOKEN_INVALID ->
                            HttpStatus.UNAUTHORIZED
                    ErrorCode.EMAIL_DUPLICATE -> HttpStatus.CONFLICT
                    ErrorCode.INVALID_REQUEST -> HttpStatus.BAD_REQUEST
                    ErrorCode.DUPLICATE_ATTEMPT -> HttpStatus.CONFLICT
            ...
            when (error) {
                is FieldError -> {
                    ...
                    fieldErrors[fieldName] =
                            fieldErrors.getOrDefault(fieldName, emptyList()) + errorMessage
                }
            }
        }
